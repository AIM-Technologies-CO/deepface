...
def extract_batch_faces(
    ...
    # img might be path, base64 or numpy array. Convert it to numpy whatever it is.
    img_list = [load_image(img) for img in img_path_list]
    img_region_list = [[0, 0, img.shape[1], img.shape[0]] for img in img_list]

    face_detector = FaceDetector.build_model(detector_backend)
    face_objs_list = FaceDetector.detect_batch_faces(face_detector, detector_backend, img_list, align)
    for face_objs, img, img_region in zip(face_objs_list, img_list, img_region_list):
        extracted_faces = []
        # in case of no face found
        if len(face_objs) == 0 and enforce_detection is True:
            raise ValueError(
        ...
        extracted_faces_list.append(extracted_faces)
    return extracted_faces_list


def resize_faces(face_objs_list, img_list, img_region_list, target_size, enforce_detection, grayscale):
    extracted_faces_list = []
    for face_objs, img, img_region in zip(face_objs_list, img_list, img_region_list):
        extracted_faces = []
        # in case of no face found
...
def extract_multi_model_faces(
    ...
    Args:
        img: a path, url, base64 or numpy array.
        target_size dictionary: the target size (W, H) of the extracted faces refrenced with the modelname
        detector_backend (str, optional): the face detector backend. Defaults to "opencv".
        grayscale (bool, optional): whether to convert the extracted faces to grayscale.
        Defaults to False.
        enforce_detection (bool, optional): whether to enforce face detection. Defaults to True.
    ...
    face_detector = FaceDetector.build_model(detector_backend)
    face_objs_list = FaceDetector.detect_batch_faces(face_detector, detector_backend, img_list, align)
    for model_name, target_size in target_size_obj.items():
        extracted_faces_obj[model_name] = resize_faces(face_objs_list, img_list, img_region_list, target_size, enforce_detection, grayscale)
    return extracted_faces_obj
